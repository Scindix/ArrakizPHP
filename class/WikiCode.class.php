...
class WikiCode
{
	...
	{
		$this->pCodeArr = explode("\n\n", $code);
		foreach($this->pCodeArr as $c)
		{
			$c = "\n\n\n$c\n\n@EOF@**";
		}
		$this->pCode = "\n\n\n$code\n\n@EOF@**";
	...
	public function parse()
	{
		...
		//{
			//$this->pCode = $c;
			//$this->pCode = $this->replace("  ", " ", $this->pCode);
			$this->pCode = $this->replace("\r", "\n", $this->pCode);
			//$this->pCode = $this->replace(" \n", "\n", $this->pCode);
			//$this->pCode = $this->replace("\n ", "\n", $this->pCode);
		
			//$c = $this->languageArray["Bold"][2];
			//$this->pCode = $c($this->pCode);
			$this->index = 0;
			/*$out .= "<p>".$this->parseR("@EOF@")."</p>";*/return $this->parseR("@EOF@");
		//}
		//return $out;
	}
	private function parseR($escapeSeq)
	{
		...
		$posNext = $this->reInitArray($this->index);
		$retVal = "";
		while($posNext < $posEscape)
		{
			$retVal .= $this->parseLimited(substr($this->pCode, $this->index, $posNext-$this->index));
			$this->index = $posNext+1;
			$retVal .= $this->searchArray[$posNext-100000][2]();
			if($this->index > strlen($this->pCode))	break;
			$posEscape = strpos($this->pCode, $escapeSeq, $this->index);
			$posNext = $this->reInitArray($this->index);
		}
		$retVal .= $this->parseLimited(substr($this->pCode, $this->index, $posEscape-$this->index));
		$this->index = $posNext+strlen($escapeSeq);
		return $retVal;
	}
	private function reInitArray($begin)
	{
		$this->searchArray = array();
		foreach($this->languageArray as $key => $value)
		{
			//$this->languageArray[$key][3] = strpos($this->pCode, $value[1]);
			$pos = strpos($this->pCode, $value[0], $begin);
		//echo substr($this->pCode, $begin, $pos-$begin); echo"<br>|";
			if($pos !== false)
			{
				//if(isset($this->searchArray[$pos-100000]))
				//{
				
				//}
				//else
					$this->searchArray[$pos-100000] = $this->languageArray[$key];
			}
		}
		$arr = array_keys($this->searchArray);
		if(empty($arr))
			$min=NULL;
		else
			$min = min($arr);
